...
class Result(models.Model):
    percent = models.DecimalField(verbose_name="Процент прохождения/Completion percent", editable=False)
    test_result = models.BooleanField(verbose_name="Статус теста/Test status", editable=False, default=False)
    student = models.OneToOneField(Student, on_delete="PROTECT")
    course = models.OneToOneField(Course, on_delete="PROTECT")